...
def build_deb(target_arch="amd64"):
    ...
    # İzinleri düzelt (opt içindeki scriptler için)
    possible_scripts = ["ro-control-root-task"]
    for s in possible_scripts:
        sp = os.path.join(opt_dir, "data", s)
    ...
    # Changelog (Lintian fix)
    import gzip
    changelog_path = os.path.join(doc_dir, "changelog.gz")
    with gzip.open(changelog_path, "wt", encoding="utf-8") as f:
        f.write(f"{app_name} ({version}) unstable; urgency=low\n\n")
        f.write(AppConfig.CHANGELOG)
        f.write(f"\n -- {AppConfig.MAINTAINER}  Wed, 11 Dec 2025 12:00:00 +0300\n")
    ...
    control_lines = [
        ...
        f"Description: Linux GPU Driver Manager",  # Kısa özet (Synopsis)
        f" {AppConfig.DESCRIPTION}", # Uzun açıklama (girintili olmalı)
        " .",
